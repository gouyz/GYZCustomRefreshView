...
@interface GYZSyncSpinnerRing : CALayer
...
@implementation GYZSyncSpinnerRing
...
- (void)layoutSublayers
{
    ...
    _arcLayer.lineWidth = size.width * 0.09;
...
@interface GYZSyncSpinner ()

@property (strong, nonatomic) GYZSyncSpinnerRing *topRing;
...
- (void)initLayers
{
    ...
    _topRing = [[GYZSyncSpinnerRing alloc] init];
...
- (void)startAnimating
{
    ...
    rotation.autoreverses = NO;//当你设定这个属性为 YES 时,在它到达目的地之后,动画的返回到开始的值,代替了直接跳转到 开始的值。
...
- (void)performCompletion
{
    ...
    [CATransaction setCompletionBlock:^{
        self.topRing.arcLayer.strokeColor = [UIColor colorWithRed:16./255 green:119./255 blue:234./255 alpha:1.0f].CGColor;
        self.topRing.arrowLayer.fillColor = [UIColor colorWithRed:16./255 green:119./255 blue:234./255 alpha:1.0f].CGColor;
...
- (void)showCheckmark
{
    ...
    CGPoint startPoint = CGPointMake(0.0, size.height*0.1+1                                                                                         );
    CGPoint midPoint = CGPointMake(size.width*0.4, size.height*0.8+1);
    ...
    checkmarkLayer.lineWidth = size.width * 0.1;
